...
export default function NavigationPage() {
  ...
  // Initialize map
  useEffect(() => {
    if (!mapLoaded || !mapRef.current || mapLoadError || map || typeof window === "undefined") return
  ...
  useEffect(() => {
    ...
    if (!map || !directionsRenderer || routeCalculated || mapLoadError) return
    ...
    try {
      ...
      directionsService.route(request, (result, status) => {
        if (status === window.google.maps.DirectionsStatus.OK && result) {
          ...
          // Extract the first step instructions if available
          if (result.routes && result.routes[0] && result.routes[0].legs && result.routes[0].legs[0]) {
            const firstStep = result.routes[0].legs[0].steps[0]
            if (firstStep) {
              setCurrentStep({
                instruction: firstStep.instructions.replace(/<[^>]*>/g, ""), // Remove HTML tags
  ...
  }, [map, directionsRenderer, routeCalculated, mapLoadError, destination.position])
  ...
  }, [map, mapLoadError]) // Only re-run if map changes

  const handleExitRoute = () => {
    // Navigate to Fred Tucker's patient page (ID: 1)
